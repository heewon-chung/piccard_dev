    ...
    def invoke_fake_runner(self, fault: str | None = None, *, source_snapshot: Path | None = None,
        ...
        self.write(scripts / "run_pre_threshold_profiles.sh", """
            ...
            for i,(name,args) in enumerate(specs):
                ...
                out={'csv':str(p.relative_to(root)),'csv_sha256':hashlib.sha256(p.read_bytes()).hexdigest(),'log':str(q.relative_to(root)),'log_sha256':hashlib.sha256(q.read_bytes()).hexdigest(),'expected_csv_rows':12 if i==0 else 1,'csv_row_count':12 if i==0 else 1,'measurement_output':'measured-csv'}
                if i==0:
