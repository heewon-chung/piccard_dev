...
3. Enumerate and run focused Work 1–6 and Work 7 tests.
4. Run one toy estimator probe.
...
10. Hash artifacts and write the canonical session manifest.

## Success conditions

1. Configuration and compilation succeed from the fresh directory.
2. Every required focused test is present and passes without a skip.
...
6. All required artifacts appear once in the manifest with matching SHA-256.
7. A second manifest verification pass succeeds without rewriting artifacts.

## Failure conditions

- A stale or pre-existing build directory is reused.
- A required binary/test is absent, skipped, or fails.
...
- An artifact is empty, malformed, duplicated, unhashed, or outside the session.
...
Tests inspect command construction before execution, enforce count `1`, reject
actual-data paths, and exercise manifest tampering. The authoritative smoke run
executes each probe exactly once. Timing values may be recorded as diagnostics
but are not compared to performance thresholds.
